...
results_path = (
    "/data/ANHIR_Out_Aff_1024_Masks_TRANSFORMED"  # Path to the results (from main.py)
)
submission_path = "/data/ANHIR_Sub_Aff_1024_Masks_TRANSFORMED_Exp_0"  # Path where submission should be saved
...
def prepare_submission():
    ...
    for current_id in ids:
        ...
        source_landmarks_path = os.path.join(
            results_path, current_id, "source_landmarks.csv"
        )
        transformed_source_landmarks_path = os.path.join(
            results_path, current_id, "transformed_source_landmarks.csv"
        )
        target_landmarks_path = os.path.join(
            results_path, current_id, "target_landmarks.csv"
        )
        time_path = os.path.join(results_path, current_id, "time.txt")
        source_landmarks = utils.load_landmarks(source_landmarks_path)
        transformed_source_landmarks = utils.load_landmarks(
            transformed_source_landmarks_path
        )
        try:
        ...
        if status == "training":
            print(
                "Resampled median initial rTRE: ",
                np.median(
                    utils.calculate_rtre(
                        source_landmarks,
                        target_landmarks,
                        np.sqrt(source_size[0] ** 2 + source_size[1] ** 2),
                    )
                ),
            )
            print(
                "Resampled median final rTRE: ",
                np.median(
                    utils.calculate_rtre(
        ...
        _, _, _, _, new_shape = calculate_pad_size(
            original_source_size, original_target_size
        )
        resample_ratio = max(new_shape) / max(source_size)

        org_source_landmarks = source_landmarks.copy()
        org_source_landmarks[:, 0] = org_source_landmarks[:, 0] * resample_ratio - (
            int(np.floor((new_shape[0] - original_source_size[0]) / 2))
        )
        org_source_landmarks[:, 1] = org_source_landmarks[:, 1] * resample_ratio - (
            int(np.floor((new_shape[1] - original_source_size[1]) / 2))
        )

        org_transformed_source_landmarks = transformed_source_landmarks.copy()
        org_transformed_source_landmarks[:, 0] = org_transformed_source_landmarks[
            :, 0
        ] * resample_ratio - (
            int(np.floor((new_shape[0] - original_target_size[0]) / 2))
        )
        org_transformed_source_landmarks[:, 1] = org_transformed_source_landmarks[
            :, 1
        ] * resample_ratio - (
            int(np.floor((new_shape[1] - original_target_size[1]) / 2))
        )

        if status == "training":
            org_target_landmarks = target_landmarks.copy()
            org_target_landmarks[:, 0] = org_target_landmarks[:, 0] * resample_ratio - (
                int(np.floor((new_shape[0] - original_target_size[0]) / 2))
            )
            org_target_landmarks[:, 1] = org_target_landmarks[:, 1] * resample_ratio - (
                int(np.floor((new_shape[1] - original_target_size[1]) / 2))
            )
            i_tre = np.median(
                utils.calculate_rtre(
                    org_source_landmarks, org_target_landmarks, diagonal
                )
            )
            f_tre = np.median(
                utils.calculate_rtre(
                    org_transformed_source_landmarks, org_target_landmarks, diagonal
                )
            )
        ...
        utils.save_landmarks_submission(
            org_transformed_source_landmarks,
            os.path.join(submission_path, transformed_path),
        )
        utils.save_landmarks_submission(
            org_source_landmarks, os.path.join(submission_path, org_src_path)
        )
        if status == "training":
            org_trg_path = os.path.join(current_id, "org_target_landmarks.csv")
            utils.save_landmarks_submission(
                org_target_landmarks, os.path.join(submission_path, org_trg_path)
            )

        dataframe["Execution time [minutes]"][int(current_id)] = str(
            float(execution_time) / 60
        )
...
def calculate_pad_size(source_size, target_size):
    ...
    source_y_pad = (
        (int(np.floor((new_shape[0] - y_size_source) / 2))),
        int(np.ceil((new_shape[0] - y_size_source) / 2)),
    )
    source_x_pad = (
        (int(np.floor((new_shape[1] - x_size_source) / 2))),
        int(np.ceil((new_shape[1] - x_size_source) / 2)),
    )
    target_y_pad = (
        (int(np.floor((new_shape[0] - y_size_target) / 2))),
        int(np.ceil((new_shape[0] - y_size_target) / 2)),
    )
    target_x_pad = (
        (int(np.floor((new_shape[1] - x_size_target) / 2))),
        int(np.ceil((new_shape[1] - x_size_target) / 2)),
    )
