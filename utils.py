...
def normalize_advanced(image, p_lower=10, p_upper=98, ksize=5):
    median_blurred_image = cv2.medianBlur(
        cv2.resize((image * 255).astype("uint8"), (512, 512)), ksize=ksize
    )
...
def calculate_tre(source_landmarks, target_landmarks):
    tre = np.sqrt(
        np.square(source_landmarks[:, 0] - target_landmarks[:, 0])
        + np.square(source_landmarks[:, 1] - target_landmarks[:, 1])
    )
    return tre
...
def numpy_affine_transform(image, transform, cval=0.0):
    ...
    u[0, :, :] = (
        transform[0, 0] * grid_x + transform[0, 1] * grid_y + transform[0, 2] - grid_x
    )
    u[1, :, :] = (
        transform[1, 0] * grid_x + transform[1, 1] * grid_y + transform[1, 2] - grid_y
    )
    transformed_image = np.empty(shape)
    if len(image.shape) == 3:
        for i in range(shape[2]):
            transformed_image[:, :, i] = numpy_warp_image(
                image[:, :, i], u[0], u[1], cval=cval
            )
    else:
...
def load_pair(case_id, dataset_path, load_masks=False):
    ...
    # sitk.GetImageFromArray((utils.normalize(resampled_target).astype(np.float32)))
    ...
    if load_masks:
        source_mask = sitk.GetArrayFromImage(sitk.ReadImage(source_mask_path)).astype(
            np.float32
        )
        target_mask = sitk.GetArrayFromImage(sitk.ReadImage(target_mask_path)).astype(
            np.float32
        )
...
def load_pair_registered(case_id, dataset_path, load_masks=False):
    base_path = os.path.join(dataset_path, str(case_id))
    source_path = os.path.join(base_path, "source.mha")
    target_path = os.path.join(base_path, "transformed_target.mha")
    if load_masks:
        source_mask_path = os.path.join(base_path, "source_mask.mha")
        target_mask_path = os.path.join(base_path, "transformed_target_mask.mha")
    source_landmarks_path = os.path.join(base_path, "transformed_source_landmarks.csv")
    target_landmarks_path = os.path.join(base_path, "target_landmarks.csv")
...
def warp_tensor(tensor, displacement_field, device="cpu"):
    ...
    transformed_tensor = F.grid_sample(
        tensor.view(1, 1, y_size, x_size),
        n_grid.view(1, n_grid.size(0), n_grid.size(1), n_grid.size(2)),
        mode="bicubic",
        padding_mode="zeros",
    )[0, 0, :, :]
...
def warp_tensors(tensors, displacement_fields, device="cpu"):
    ...
    n_grid_x = (
        grid_x.view(1, -1)
        .repeat(no_samples, 1)
        .view(-1, grid_x.size(0), grid_x.size(1))
    )
    n_grid_y = (
        grid_y.view(1, -1)
        .repeat(no_samples, 1)
        .view(-1, grid_y.size(0), grid_y.size(1))
    )
    ...
    transformed_tensors = F.grid_sample(
        tensors, n_grid, mode="bicubic", padding_mode="zeros"
    )
...
def resample_tensor(tensor, new_size, device="cpu"):
    ...
    n_grid = torch.stack((n_grid_x, n_grid_y), dim=3)
    resampled_tensor = F.grid_sample(
        tensor.view(1, 1, tensor.size(0), tensor.size(1)),
        n_grid,
        mode="bicubic",
        padding_mode="zeros",
    )[0, 0, :, :]
...
def resample_tensors(tensors, new_size, device="cpu"):
    ...
    n_grid = torch.stack((n_grid_x, n_grid_y), dim=3)
    resampled_tensors = F.grid_sample(
        tensors, n_grid, mode="bicubic", padding_mode="zeros"
    )
...
def transform_to_displacement_field(tensor, tensor_transform, device="cpu"):
    ...
    displacement_field = torch.cat(
        (u_x.view(1, y_size, x_size), u_y.view(1, y_size, x_size)), dim=0
    )
...
def upsample_displacement_fields(displacement_fields, new_size, device="cpu"):
    ...
    n_grid = torch.stack((n_grid_x, n_grid_y), dim=3)
    resampled_displacement_fields = F.grid_sample(
        displacement_fields, n_grid, mode="bilinear", padding_mode="zeros"
    )
...
def upsample_displacement_field(displacement_field, new_size, device="cpu"):
    ...
    resampled_displacement_field = F.grid_sample(
        displacement_field.view(
            1, 2, displacement_field.size(1), displacement_field.size(2)
        ),
        n_grid,
        mode="bilinear",
        padding_mode="zeros",
...
def tensor_laplacian(tensor, device="cpu"):
    laplacian_filter = torch.Tensor([[-1, -1, -1], [-1, 8, -1], [-1, -1, -1]]).to(
        device
    )
...
def fold(
    unfolded_tensor, padded_output_size, padding_tuple, patch_size, stride, device="cpu"
):
    new_tensor = torch.zeros(
        (
            1,
            unfolded_tensor.size(1),
        )
        + padded_output_size
    ).to(device)
    col_y, col_x = int(padded_output_size[0] / stride - 1), int(
        padded_output_size[1] / stride - 1
    )
    for j in range(col_y):
        for i in range(col_x):
            current_patch = unfolded_tensor[
                j * col_x + i,
                :,
    ...
    if padding_tuple[2] == 0 and padding_tuple[3] == 0:
        ...
        new_tensor = new_tensor[
            :, :, padding_tuple[1] : -padding_tuple[3], padding_tuple[0] :
        ]
    elif padding_tuple[3] == 0:
        new_tensor = new_tensor[
            :, :, padding_tuple[1] :, padding_tuple[0] : -padding_tuple[2]
        ]
    else:
        new_tensor = new_tensor[
            :,
            :,
            padding_tuple[1] : -padding_tuple[3],
            padding_tuple[0] : -padding_tuple[2],
        ]
    return new_tensor
...
def unfold(tensor, patch_size, stride, device="cpu"):
    ...
    b_x, e_x = (
        math.floor(pad_x / 2) + patch_size[0],
        math.ceil(pad_x / 2) + patch_size[0],
    )
    b_y, e_y = (
        math.floor(pad_y / 2) + patch_size[1],
        math.ceil(pad_y / 2) + patch_size[1],
    )
        ...
        if i == num_levels - 1:
            ...
            new_size = torch.Size(
                (
                    current_size[0],
                    current_size[1],
                    int(current_size[2] / (2 ** (num_levels - i - 1))),
...
def stn(src, flow, mode="bicubic", corners=False, pad="zeros"):
    ...
    return F.grid_sample(
        src, new_locs, align_corners=corners, mode=mode, padding_mode=pad
    )


def post_processing_mask(mask):
    mask = nd.binary_fill_holes(mask, structure=np.ones((3, 3))).astype(int)
